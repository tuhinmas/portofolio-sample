...
class ExportDirectDistributor implements FromCollection, WithHeadings, WithMapping
{
    use Exportable;
    use SalesOrderTrait;
    ...
    public function collection()
    {
        ...
        $sales_orders = SalesOrder::query()
            ...
            ->whereHas("dealer", function ($QQQ) use ($year) {
                return $QQQ->whereHas("ditributorContract", function ($QQQ) use ($year) {
                    return $QQQ
            ...
            ->filter(fn($order) => $this->orderInsideContractDistributor($order))
            ->groupBy([
